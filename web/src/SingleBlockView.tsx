...
import { type TrajectorySample, planAndSample } from "./wasm/planner_wasm_loader";
...
export default function SingleBlockView() {
  ...
  function handleShiftStep(
    ...
    const clamped = Math.min(
      Number(el.max || Number.POSITIVE_INFINITY),
      Math.max(Number(el.min || Number.NEGATIVE_INFINITY), current + delta)
    );
  ...
  useEffect(() => {
    ...
    const { times, position, velocity, acceleration, jerk, duration } =
      planState.sample;
  ...
  async function runPlan(planInputs: InputState) {
    ...
    try {
      ...
      setPlanState({
        sample,
        ok,
        error: ok ? null : "Planner returned zero duration.",
      });
    } catch (err) {
      ...
      <section className="rounded-xl border border-[#333] bg-[#1d1d1d] px-5 py-4">
        <div className="grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(200px,1fr))]">
          <div className="grid gap-1.5 text-left">
            <label className="text-sm text-[#b8b8b8]" htmlFor="entryV">
              Entry velocity (mm/s)
            </label>
            <input
          ...
          <div className="grid gap-1.5 text-left">
            <label className="text-sm text-[#b8b8b8]" htmlFor="exitV">
              Exit velocity (mm/s)
            </label>
            <input
              className="rounded-md border border-[#333] bg-[#111] px-2 py-2 text-white"
          ...
          <div className="grid gap-1.5 text-left">
            <label className="text-sm text-[#b8b8b8]" htmlFor="maxEntry">
              Max entry speed (mm/s)
            </label>
            <input
              className="rounded-md border border-[#333] bg-[#111] px-2 py-2 text-white"
              id="maxEntry"
          ...
          <div className="grid gap-1.5 text-left">
            <label className="text-sm text-[#b8b8b8]" htmlFor="distance">
              Distance (mm)
            </label>
            <input
              className="rounded-md border border-[#333] bg-[#111] px-2 py-2 text-white"
          ...
          <div className="grid gap-1.5 text-left">
            <label className="text-sm text-[#b8b8b8]" htmlFor="aMax">
              A max (mm/s²)
            </label>
            <input
              className="rounded-md border border-[#333] bg-[#111] px-2 py-2 text-white"
          ...
          <div className="grid gap-1.5 text-left">
            <label className="text-sm text-[#b8b8b8]" htmlFor="jMax">
              J max (mm/s³)
            </label>
            <input
              className="rounded-md border border-[#333] bg-[#111] px-2 py-2 text-white"
          ...
          <div className="grid gap-1.5 text-left">
            <label className="text-sm text-[#b8b8b8]" htmlFor="dt">
              Sample dt (s)
            </label>
            <input
              className="rounded-md border border-[#333] bg-[#111] px-2 py-2 text-white"
          ...
          <div className="rounded-lg border border-[#2a2a2a] bg-[#121212] p-3">
            ...
            <p className="text-lg">
              {format(planState.sample?.position.at(-1) ?? 0)}
            </p>
          </div>
          <div className="rounded-lg border border-[#2a2a2a] bg-[#121212] p-3">
          ...
          <div className="rounded-lg border border-[#2a2a2a] bg-[#121212] p-3">
            ...
            <p className="text-lg">
              {planState.sample?.status || "Not planned"}
            </p>
          </div>
          <div className="rounded-lg border border-[#2a2a2a] bg-[#121212] p-3">
            <h4 className="mb-2 text-sm text-[#9aa3b2]">Plan time</h4>
            <p className="text-lg">
              {format(planState.sample?.planTimeUs ?? 0, 3)} &micro;s
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 flex items-center gap-3">
          {planState.error ? (
            <span className="text-sm text-[#9aa3b2]">{planState.error}</span>
          ) : null}
        </div>
      </section>
